...
import { buildEstimate, estimateToAOA } from '../src/geometry/estimateExport';
...
const aoa = estimateToAOA(doc, { title: '水上様邸', date: '2025-09-24' });
ok(aoa[0][0] === '御 見 積 書', 'ヘッダ 御見積書');
ok(aoa[4].join(',') === '品名,数量,単位,単価,金額,摘要', '見出し 品名/数量/単位/単価/金額/摘要');
ok(aoa.some((r) => r[3] === '合計' && r[4] === doc.total), '合計行あり');
